...
private:
	...
	void processOneCommand(String cmdStr){
		for(int i=0; i<numberOfCommands; i++){
			...
			if(listOfCommands[i]->isKey(extractCommand(cmdStr))){
				Serial.println("Found Match");
				Serial.println(listOfCommands[i]->key);
				listOfCommands[i]->executeCMD(cmdStr,errors);
				break;
			}
		}
	}
...
public:
	SCPI_Parser(SCPI_Command** cmdList, int numberOfCmds) {
		listOfCommands = cmdList;
		numberOfCommands = numberOfCmds;
	}
	~SCPI_Parser() {
